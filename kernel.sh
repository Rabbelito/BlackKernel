...
#make vendor/sweet2.config O=out
...
export dtb="$PWD"/out/arch/arm64/boot/dtb.img 
		...
		if [ "$DEVICE_TYPE" = courbet  ]; then
                ...
                find out/ -name \*.ko -exec cp {} zip/modules/system/lib/modules \;
                #cp -r out/**/*.ko zip/modules/system/lib/modules
                
                cd zip
                export ZIP="$KERNEL_NAME"-"$KRNL_REL_TAG"-"$CODENAME"
                zip -r9 "$ZIP" * -x .git README.md LICENSE *placeholder placeholder
